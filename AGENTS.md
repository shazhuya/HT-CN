...
CR-0090 恢复时先读 `specs/m9-recognition-gate4c-correctness-first.md`。
顺序是事件合同与 PRZ 案例核查、真实标签及新封存验收、容量选择、单接口生产接入。
不得从历史 Gate 4B green 或容量排序推断识别已可信。已用于调参的 holdout 不再是最终盲验。
每个工作单元交付可核对的案例、合同或识别修复及证据，禁止以外围工作或重复计数报告替代。
