...
def main() -> int:
    parser = argparse.ArgumentParser(description="HT-CN M3 industry context sync")
    ...
    parser.add_argument("--output", default="artifacts/reports/m3-industry-context-sync.json")
    ...
    payload = {
        ...
        "sector_snapshot_count": len(snapshots),
        "aggregate_method": "local_constituent_equal_weight_mean_and_median",
        "source_boundary": (
            "Eastmoney supplies membership only; strength/breadth/volume are "
            "recomputed from local M1 constituent bars."
        ),
    }
