...
即使外部复现再次 `confirmed`，也只能说：在另一组独立股票历史样本中，同一 source-aligned T+5 price-action 关系得到复现。仍然不能把结果写成：
...
真正进入实战工具时，它仍只能作为 lifecycle/evidence 层，与 pattern identity、市场环境、风险管理分离。
