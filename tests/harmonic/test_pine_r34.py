...
def test_r34_full_scan_gartley_locks_nodes_birth_and_projected_prz():
    ...
    # Formula-lock fixture uses wick=0 so the expected X/A/B/C prices are exact.
    # Separate pivot tests lock the fact that real pivots use High/Low rather than Close.
    assert [node.price for node in candidate.source_nodes] == pytest.approx(
        [100.0, 200.0, 138.2, 183.2]
    )
...
def test_r34_precise_abcd_full_scan_keeps_two_measurement_convergence():
    ...
    assert candidate.m2 == pytest.approx(69.10)
    assert candidate.prz_low == pytest.approx(69.10)
