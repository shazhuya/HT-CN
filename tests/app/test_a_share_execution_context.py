...
def test_star_context_exposes_t_plus_one_atr_and_20pct_board_limit() -> None:
    ...
    assert context.exact_price_limit_pct == pytest.approx(20.0)
    assert context.price_limit_status == "exact_current_board_profile_with_known_listing_metadata"
...
def test_main_board_risk_warning_limit_switch_is_date_gated_at_2026_07_06() -> None:
    ...
    assert before.exact_price_limit_pct == pytest.approx(5.0)
    ...
    assert after.exact_price_limit_pct == pytest.approx(10.0)
    assert after.price_limit_status == "exact_current_board_profile_with_known_listing_metadata"


def test_first_five_listing_sessions_fail_open_for_exact_limit_and_keep_exception_flag() -> None:
    ...
    assert context.nominal_price_limit_pct == pytest.approx(20.0)
    assert context.exact_price_limit_pct is None
